...
def main():
    ...
    if len(sys.argv) < 2:
        print "ERROR: Need to Specify Output File Name as Argument!!!"
        return
    else:
        print "Specified Ouptut File = " + str(sys.argv[1])[0:-4] + ".csv"

    out_file_name = str(sys.argv[1])[0:-4] + ".csv"
    out_file = open(out_file_name, "w")
    out_file.write("FPGA_temperature\tADC_1.0V\tADC_1.0V_AVCC\tADC_1.2V_AVTT\tADC_1.8V\tADC_1.5V\tADC_2.5V\tADC_RSSI1\tADC_RSSI2\tFEAST1_I\tFEAST2_I\tFEAST3_I\tFEAST4_I\tFEAST5_I\tU2_TEMP\tU3_TMEP\tU22_TEMP\tU8_TEMP\tCURR_MIR_CAL\tSCA_TEMP\n")
    ...
    while(iter == 0):
        ...
        data_csv = "%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t" % (temp, adc_1v0, adc_1v0avcc, adc_1v2avtt, adc_1v8, adc_1v5, adc_2v5, adc_rssi1_ua, adc_rssi2_ua, adc_feast1_a, adc_feast2_a, adc_feast3_a, adc_feast4_a, adc_feast5_a, adc_u2_temp, adc_u3_temp, adc_u22_temp, adc_u8_temp, curr_Mirror_Cal, adc_sca_temp)
        out_file.write("%s\n" % data_csv)
        out_file.flush()
